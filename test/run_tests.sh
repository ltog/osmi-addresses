...
./test_engine.pl $1 "Number of 'no alphabetic part in addr:housenumber' errors in osmi_addresses_interpolation" "SELECT COUNT(*) FROM osmi_addresses_interpolation WHERE error=\"no alphabetic part in addr:housenumber\";" "=" "2"
./test_engine.pl $1 "Number of 'numeric parts of housenumbers not identical' errors in osmi_addresses_interpolation" "SELECT COUNT(*) FROM osmi_addresses_interpolation WHERE error=\"numeric parts of housenumbers not identical\";" "=" "1"
./test_engine.pl $1 "Number of 'endpoint has wrong format' errors in osmi_addresses_interpolation" "SELECT COUNT(*) FROM osmi_addresses_interpolation WHERE error=\"endpoint has wrong format\";" "=" "4"
