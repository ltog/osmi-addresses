...
public:
	CompressedWay(std::unique_ptr<OGRLineString> way) {
		big_coord_pair sum;
		num_points = way->getNumPoints();
		first.x = sum.x = way->getX(0);
		first.y = sum.y = way->getY(0);
		...
		for (int i=1; i<=num_points-2; i++) { // i is index of way, NOT intermediates!
			deltas[i-1].x = way->getX(i) - sum.x;
			deltas[i-1].y = way->getY(i) - sum.y;
			sum.x += deltas[i-1].x;
			sum.y += deltas[i-1].y;
		}

		last.x = way->getX(num_points-1);
		last.y = way->getY(num_points-1);
	}
	...
	std::unique_ptr<OGRLineString> uncompress() {
		...
		linestring->addPoint(first.x, first.y);
		big_coord_pair prev;
		prev.x = first.x;
		prev.y = first.y;
		for (int i=0; i<=num_points-3; i++) {
			linestring->addPoint(prev.x + deltas[i].x, prev.y + deltas[i].y);
			prev.x += deltas[i].x;
			prev.y += deltas[i].y;
		}
		linestring->addPoint(last.x, last.y);
