...
public:
	...
	NearestAreasWriter(const std::string& dir_name) :
		Writer(dir_name, "osmi_addresses_nearest_areas", USE_TRANSACTIONS, wkbPolygon)  {
	...
	void write_area(
		...
		if (written_ways.find(way_id) == written_ways.end()) {
			...
			feature->SetField("name", addrarea.c_str());
